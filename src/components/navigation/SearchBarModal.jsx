                  ...
                  />
                    <div style = {{display:"flex", justifyContent:"space-between"}}>
                      ...
                      postInfo.attending.some(attending => attending._id === user.id) 
                        ? <FavoriteIcon sx = {{color:"red", cursor:"pointer"}} onClick = {()=> searchLikeHandler(postInfo._id)}/>
                        : <FavoriteBorderIcon onClick = {()=> searchLikeHandler(postInfo._id)} sx = {{color:"red", cursor:"pointer"}}/>
                      }
                      <div style = {{display:"flex"}}>
                      <AnimatePresence>
                      {
                        postInfo.attending.map(
                          (attending, index) => index <= 2 
                            ? <motion.div
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1}}
